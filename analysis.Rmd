...
df_lng_unique <- read_csv("data/unique_sentences_tagged_9-15.csv") %>% 
  filter(exp_id %in% complete.exp_ids,
         generation < 8) %>%
  rename(
...
df_lng_all <- read_csv("data/all_sentences_tagged_9-15.csv") %>% 
  ...
  select(sentence, generation, game_name, chain, 
         # ends_with("_label"), 
         ends_with("_elicit"),
         ends_with("_elicitProb"),
         ends_with("_pastThreshold"))
...
df_lng_all_tidy %>%
  ...
  group_by(chain, generation, game_name, tag, elicit) %>%
  count() %>%
  ungroup() %>%
  group_by(chain, generation, game_name, tag) %>%
  mutate(
    prop = n / sum(n)
