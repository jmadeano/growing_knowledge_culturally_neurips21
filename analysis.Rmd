...
df_msg_editFine <- read_csv(paste(project.path, "messages_with_diff.csv", sep = "")) %>% mutate(message_length = nchar(message)) %>%
  filter(exp_id %in% complete.exp_ids) %>%
  mutate(
...
df_lng_heldout <- read_csv("data/ought-classification_heldout.csv") %>% 
  filter(exp_id %in% complete.exp_ids,
         generation < 8)

df_lng_combined <- bind_rows(
  #df_lng_labeled, 
  df_lng_heldout
) %>%
  rename(
     # "dynamics_label" = "dynamics -- Your Tag",
    "dynamics_elicit" = "dynamics -- Elicit's Tag",
    "dynamics_elicitProb" = "dynamics -- P(Elicit's tag)",
    # "policy_label" = "policy -- Your Tag",
     "policy_elicit" = "policy -- Elicit's Tag",
    "policy_elicitProb"= "policy -- P(Elicit's tag)",
     # "abstractness_label" =  "abstract -- Your Tag",
     "abstractness_elicit" = "abstract -- Elicit's Tag",
    "abstractness_elicitProb" = "abstract -- P(Elicit's tag)",  
     # "valence_label" = "valence -- Your Tag",
    "valence_elicit" = "valence -- Elicit's Tag",
     "valence_elicitProb" = "valence -- P(Elicit's tag)",
     "dynamics_pastThreshold" = `dymanics -- Elicit's Tag meets confidence threshold`,
      "policy_pastThreshold" = `policy -- Elicit's Tag meets confidence threshold`,
     "abstractness_pastThreshold" = `abstract -- Elicit's Tag meets confidence threshold`,
     "valence_pastThreshold" = `valence -- Elicit's Tag meets confidence threshold`
  ) %>%
           ...
           factor(elicit, 
                  levels = c(
                    ...
                    "winning, including mentions of scoring points, victory, success, goals, solutions, best strategies"), 
                         labels = c("dynamics", "NOTdyanmics", "policy", "NOTpolicy", "ignorance", "abstract", "concrete", "neutral", "loss", "win"))) -> df_lng_tidy

df_lng_summ <- df_lng_tidy %>%
...
```{r}
ggplot(df_lng_summ, aes(x = generation, 
...
df_lng_combined %>%
  ...
  mutate(combined_tag = case_when(
    ...
    combined_tag == "abstract_not dynamics_policy" ~ "concrete_policy",
    TRUE ~ combined_tag)
    ) %>%
  filter(!grepl("not dynamics_not policy", combined_tag, fixed = TRUE)) -> df_lng_combined_combo
```
...
```{r}
df_lng_combo_summ <- df_lng_combined_combo %>%
...
ggplot(df_lng_combo_summ, aes(x = generation, 
...
```{r}
df_lng_valence_summ <- df_lng_combined_combo %>%
...
ggplot(df_lng_valence_summ, aes(x = generation, 
