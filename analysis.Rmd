...
```
#### Edits vs. generation + total_levels_won

```{r}
brm(
  edit_distance ~ gen_cent + frac_levels_cent + 
    (1 + gen_cent + frac_levels_cent | game_name) + 
    (1 + gen_cent + frac_levels_cent | chain),
  data = df_game_summary_edit %>%
    ungroup() %>%
    mutate(
      gen_cent = generation - mean(generation),
      frac_levels_cent = fraction_levels_completed - mean(fraction_levels_completed)
    ),
 family = gaussian(),
 chains = 4,
 iter = 3000,
 cores = 4  
) -> rs.brm_edit_gen_cent

summary(rs.brm_edit_gen_cent)
```
...
## Unique Sentences
```{r}
df_lng_unique <- read_csv("data/all_sentences_tagged_9-15.csv") %>%
  arrange(chain, generation, game_name) %>% 
  distinct(chain, game_name, sentence, .keep_all = TRUE) %>% 
  filter(exp_id %in% complete.exp_ids,
         generation < 8) %>%
  rename(
...
df_lng_unique %>%
  mutate(dynamics_elicit = 
         ...
         policy_elicit = 
           factor(policy_elicit, levels = c("policy, or what actions to take including strategies or instructions", "not policy, or what actions to take including strategies or instructions"
           ), labels = c("policy", "not policy")
           ),
         abstractness_elicit = factor(abstractness_elicit, levels = c("ignorance statements or specific experiences", "abstract, complex, high-level information",  "concrete, simple, low-level information"), labels = c("ignorance", "abstract", "concrete")),
...
df_lng_unique_combo_summ <- df_lng_unique_combo %>%
 ...
 tidyboot(
  ...
  ungroup() %>%
  mutate(
    combined_tag = factor(combined_tag, 
                          levels = c("abstract_policy",
                                     "concrete_policy",
                                     "ignorance",
                                     "concrete_dynamics_not policy",
                                     "abstract_dynamics_not policy"),
                          labels = c(
                                                                 "Abstract Policy",
  "Concrete Policy",
  ...
  bind_rows(
    data.frame(
      generation = c(6, 7),
      combined_tag = "Ignorance/Experience",
      n = c(0, 0),
      empirical_n = c(0, 0),
      empirical_stat = c(0, 0),
      ci_lower = c(0.01, 0.01),
      mean = c(0.01, 0.01),
      ci_upper = c(0.01, 0.01)
    )
  )

df_lng_unique_combo_last <- df_lng_unique_combo_summ %>% 
    group_by(combined_tag) %>%
    summarize(
        last = dplyr::last(mean)
    ) %>%
  ungroup() %>%
  mutate(
    last = case_when(
      # combined_tag == "Ignorance/Experience" ~ 0,
      # combined_tag == "Abstract Dynamics" ~ 0.04,
      combined_tag == "Abstract Dynamics" ~ 10,
      combined_tag == "Concrete Policy" ~ 70,
      # combined_tag == "Concrete Policy" ~ 0.45,
      # combined_tag == "Concrete Dynamics" ~ 0.4,
      combined_tag == "Concrete Dynamics" ~ 60,
      TRUE ~ last
      ))


ggplot(df_lng_unique_combo_summ, aes(x = generation + 1, 
  ...
  scale_y_continuous(
    ...
    expand = c(0,0),
    sec.axis = dup_axis(
      breaks = df_lng_unique_combo_last$last,
      labels = df_lng_unique_combo_last$combined_tag,
      name = NULL
    )
  ) +
  guides(color="none")+
  scale_color_manual(values = c("#ca0020", "#f4a582",
                                "#404040",
                                "#92c5de","#0571b0"))+
  # ylab("Proportion of new message content")+
  ylab("Number of new sentences")+
  xlab("Generation")

ggsave("new_lang_number_byGen.pdf", width = 4.5, height = 2.5)
```
...
df_lng_unique_valence_summ_last <- df_lng_unique_valence_summ %>% 
    group_by(valence_elicit) %>%
    summarize(
        last = dplyr::last(mean)
    ) %>%
  ungroup() %>%
  mutate(
    last = case_when(
      valence_elicit == "loss" ~ 15,
      valence_elicit == "win" ~ 2,
      TRUE ~ last
      ))

ggplot(df_lng_unique_valence_summ, aes(x = generation + 1, 
  ...
  scale_y_continuous(
    ...
    expand = c(0,0),
    sec.axis = dup_axis(
      breaks = df_lng_unique_valence_summ_last$last,
      labels = df_lng_unique_valence_summ_last$valence_elicit,
      name = NULL
    )
  ) +
  guides(color="none")+
...
ggsave("new_langVal_number_byGen.pdf", width = 3.25, height = 2.5)

```

### regression analaysis on what gets added by generation and performance

```{r}
df_lng_unique_combo %>%
  mutate_at(vars(generation, game_name, chain, combined_tag), as.factor) %>%
  dplyr::count(generation, game_name, chain, combined_tag, .drop = F) %>%
  mutate(
    generation = as.numeric(as.character(generation)),
    bool_count = ifelse( n > 0, 1, 0)
  ) -> df_lng_unique_combo_count

df_lng_unique_combo_count %>%
  pivot_wider(
    names_from = combined_tag,
    names_glue = "{combined_tag}_{.value}",
    values_from = c(n, bool_count)
  ) %>%
  left_join(
    df_game_summary %>%
      select(chain, generation, game_name, fraction_levels_completed)
  ) -> df_lng_unique_combo_count_perf_wide


# glmer(
#   abstract_policy_bool_count ~ generation + fraction_levels_completed +
#     (1 + generation + fraction_levels_completed | game_name),
#   family = 'binomial',
#   data = df_lng_unique_combo_count_perf_wide
# ) -> rs.glmer.bool.abstractpol_gen_perf
# summary(rs.glmer.bool.abstractpol_gen_perf)


brm(
  abstract_policy_bool_count ~ gen_cent + frac_levels_cent + 
    (1 + gen_cent + frac_levels_cent | game_name) + 
    (1 + gen_cent + frac_levels_cent | chain),
  data = df_lng_unique_combo_count_perf_wide %>%
    ungroup() %>%
    mutate(
      gen_cent = generation - mean(generation),
      frac_levels_cent = fraction_levels_completed -
        mean(fraction_levels_completed, na.rm = T) # ALERT! SOME NAs. NEED TO INVESTIGATE
    ),
 family = bernoulli(),
 chains = 4,
 iter = 3000,
 cores = 4  
) -> rs.brm.bool_abstractpol.gen_perf
...
```{r}
brm(
  concrete_policy_bool_count ~ gen_cent + frac_levels_cent + 
    (1 + gen_cent + frac_levels_cent | game_name) + 
    (1 + gen_cent + frac_levels_cent | chain),
  data = df_lng_unique_combo_count_perf_wide %>%
    ungroup() %>%
 ...
 iter = 3000,
 cores = 4  
) -> rs.brm.bool_concretepol.gen_perf
rs.brm.bool_concretepol.gen_perf
```
...
```{r}
brm(
  concrete_dynamics_bool_count ~ gen_cent + frac_levels_cent + 
    (1 + gen_cent + frac_levels_cent | game_name) + 
    (1 + gen_cent + frac_levels_cent | chain),
  data = df_lng_unique_combo_count_perf_wide %>%
    ungroup() %>%
    ...
    rename(concrete_dynamics_bool_count = `concrete_dynamics_not policy_bool_count`),
 family = bernoulli(),
 chains = 4,
 iter = 3000,
 cores = 4  
) -> rs.brm.bool_concretedyn.gen_perf
rs.brm.bool_concretedyn.gen_perf
```

```{r}
brm(
  ignorance_bool_count ~ gen_cent + frac_levels_cent + 
    (1 + gen_cent + frac_levels_cent | game_name) + 
    (1 + gen_cent + frac_levels_cent | chain),
  data = df_lng_unique_combo_count_perf_wide %>%
    ungroup() %>%
 ...
 iter = 3000,
 cores = 4  
) -> rs.brm.bool_ign.gen_perf
rs.brm.bool_ign.gen_perf
```


### regression analayses on what gets added by generation

```{r}
brm(
  abstract_policy_bool_count ~ gen_cent + 
    (1 + gen_cent | game_name) + 
    (1 + gen_cent | chain),
  data = df_lng_unique_combo_count_perf_wide %>%
    ungroup() %>%
    mutate(
...
```{r}
brm(
  concrete_policy_bool_count ~ gen_cent + 
    (1 + gen_cent | game_name) + 
    (1 + gen_cent | chain),
  data = df_lng_unique_combo_count_perf_wide %>%
    ungroup() %>%
...
```{r}
brm(
  concrete_dynamics_bool_count ~ gen_cent + 
    (1 + gen_cent | game_name) + 
    (1 + gen_cent | chain),
  data = df_lng_unique_combo_count_perf_wide %>%
    ungroup() %>%
 ...
 iter = 3000,
 cores = 4  
) -> rs.brm.bool_concretedyn.gen
rs.brm.bool_concretedyn.gen
```

```{r}
brm(
  ignorance_bool_count ~ gen_cent + 
    (1 + gen_cent | game_name) + 
    (1 + gen_cent | chain),
  data = df_lng_unique_combo_count_perf_wide %>%
    ungroup() %>%
  ...
  mutate(elicit = 
           factor(elicit, 
                  levels = c(
                    ...
                    "losing, including information about death, losing points, lowering scores, forfeiting, losing lives, getting stuck or trapped", 
                    "winning, including mentions of scoring points, victory, success, goals, solutions, best strategies"), 
                         labels = c("dynamics", "NOTdynamics", "policy",
                                    "NOTpolicy", "ignorance", "abstract",
                                    "concrete", "neutral", "loss", "win"))) -> df_lng_all_tidy
...
### Fig (paper) all msg by gen

```{r}

df_lng_all_combo_summ <- df_lng_all_combo_summ %>%
  ungroup() %>%
  mutate(
    combined_tag = factor(combined_tag, 
                          levels = c("abstract_policy",
...
df_lng_all_combo_last <- df_lng_all_combo_summ %>% 
    group_by(combined_tag) %>%
    summarize(
        last = dplyr::last(mean)
    )  %>%
  ...
  mutate(
    last = case_when(
      combined_tag == "Ignorance" ~ 0,
      combined_tag == "Abstract Dynamics" ~ 20,
      # combined_tag == "Concrete Policy" ~ 0.45,
      # combined_tag == "Concrete Dynamics" ~ 0.4,
      TRUE ~ last
      ))
...
ggplot(df_lng_all_combo_summ, aes(x = generation + 1, 
                        y = mean, ymin = ci_lower, ymax = ci_upper, color = combined_tag))+
  geom_linerange(alpha = 0.5)+
  geom_line(alpha = 1)+
  scale_x_continuous(expand = c(0,0),
  ...
  scale_y_continuous(
    ...
    expand = c(0,0),
    sec.axis = dup_axis(
      breaks = df_lng_all_combo_last$last,
      labels = df_lng_all_combo_last$combined_tag,
      name = NULL
    )
  ) +
  guides(color="none")+
...
ggsave("all_lang_byGen.pdf", width = 5, height = 2.75)
```
...
### All msg by game and gen

```{r}
expand.grid(
  generation = unique(df_lng_all_combo$generation),
  game_name = unique(df_lng_all_combo$game_name),
  combined_tag = unique(df_lng_all_combo$combined_tag)
) %>% bind_rows(
  df_lng_all_combo
) %>%
group_by(generation, game_name, combined_tag) %>%
 tidyboot(summary_function = function(x) x %>% 
  summarize(stat = n()),
       statistics_functions = function(x) x %>%
       summarise(across(stat, mean_ci_funs, .names = "{.fn}"))) ->  df_lng_all_game_combo_summ 

df_lng_all_game_combo_summ <- df_lng_all_game_combo_summ %>%
  ungroup() %>%
  mutate(
    combined_tag = factor(combined_tag, 
                          levels = c("abstract_policy",
...
df_lng_all_game_combo_summ %>%
  filter(combined_tag != "Ignorance") %>%
ggplot(., 
       aes(x = generation + 1, 
                        y = mean, ymin = ci_lower, ymax = ci_upper, color = combined_tag))+
  geom_linerange(alpha = 0.5)+
...
ggsave("all_lang_byGen_byGame.pdf", width = 8, height = 3.5)
...
### bayesian logistic regression with language as predictor

```{r}
df_game_lng %>%
  ungroup() %>%
  rowwise() %>%
  mutate(adjusted_level = (6 - levels_in_game) + levels_completed ) -> df_game_lng_adj
...
```{r}
# ideally we'd do ordinal regression, but for ordinal regression, you must include an intercept term, and including an intercept term changes the otherwise straightforward interpretation of the main effects

brm(
 adjusted_level ~ abstract_policy + ignorance + 
    concrete_dynamics_notpolicy + abstract_dynamics_notpolicy +
    concrete_policy + message_length +
  (1 | chain) +
  (1 | game_name),
   data = df_game_lng_adj %>%
   filter(game_name != "explore_exploit_v1") %>%
   rename(
     concrete_dynamics_notpolicy = `concrete_dynamics_not policy`,
     abstract_dynamics_notpolicy = `abstract_dynamics_not policy`
   ),
 family = cumulative("logit"),
 file = "brm_fit_language_ordinal_n80",
 # family = gaussian(),
 chains = 4,
 iter = 3000,
 cores = 4
...
brm(
 adjusted_level ~ -1 + abstract_policy + ignorance + 
    concrete_dynamics_notpolicy + abstract_dynamics_notpolicy +
    concrete_policy + message_length + 
  # (-1 + abstract_policy + ignorance + 
  #   concrete_dynamics_notpolicy + abstract_dynamics_notpolicy +
  #   concrete_policy | chain)+
  (-1 + abstract_policy + ignorance + 
    concrete_dynamics_notpolicy + abstract_dynamics_notpolicy +
    concrete_policy | game_name),
   data = df_game_lng_adj %>%
   filter(game_name != "explore_exploit_v1") %>%
   rename(
     concrete_dynamics_notpolicy = `concrete_dynamics_not policy`,
 ...
 #family = cumulative("logit"),
 # file = "brm_fit_language_gaussian_n80_noIntercept_gameMaxRE",
 family = gaussian(),
 chains = 4,
 iter = 3000,
 cores = 4
